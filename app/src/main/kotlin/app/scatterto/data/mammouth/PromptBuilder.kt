    ...
    fun system(targets: List<GenTarget>, tonality: Tonality = Tonality.DEFAULT): String {
        ...
        return buildString {
            ...
            if (targets.any { it.wantsCard }) {
                ...
                appendLine("- Grundlage sind Titel und Beschreibung unten. Übernimm sie so weit wie")
                appendLine("  möglich (nur in die Netzwerk-Sprache übertragen und kürzen); erfinde nichts dazu.")
                appendLine("- \"card_title\": knapper Titel des Artikels, höchstens rund 60 Zeichen,")
                appendLine("  ohne Hashtags, ohne Medien-Namen am Ende (kein \"… | NDR\").")
                appendLine("- \"card_description\": höchstens rund 100 Zeichen. Die Karte ist SCHMAL und wird")
                appendLine("  nach ~2 Zeilen abgeschnitten — das Wichtigste muss VORNE stehen.")
                appendLine("- KEINE Meta-Einleitung. Schreibe NICHT \"Der Artikel beschreibt …\",")
                appendLine("  \"The article describes …\", \"Der Beitrag handelt von …\" oder Ähnliches.")
                appendLine("  Beginne direkt mit der Sache selbst.")
                appendLine("  Falsch: \"Der Artikel beschreibt, wie die Preise für Streaming-Abos steigen.\"")
                appendLine("  Richtig: \"Streaming-Abos bis zu 40 % teurer in zwei Jahren.\"")
